...
def plot_line(p0: Point, p1: Point):
    if (p1.y - p0.y) == 0:
        full_line = plot_line_yequal(Point(p0.x, p0.y), Point(p1.x, p1.y))

    elif (p1.x - p0.x) == 0:
        full_line = plot_line_xequal(Point(p0.x, p0.y), Point(p1.x, p1.y))

    elif abs(p1.y - p0.y) < abs(p1.x - p0.x):
        if p0.x > p1.x:
            full_line = plot_line_low(Point(p1.x, p1.y), Point(p0.x, p0.y))
        else:
            full_line = plot_line_low(Point(p0.x, p0.y), Point(p1.x, p1.y))

    else:
        if p0.y > p1.y:
            full_line = plot_line_high(Point(p1.x, p1.y), Point(p0.x, p0.y))
        else:
            full_line = plot_line_high(Point(p0.x, p0.y), Point(p1.x, p1.y))
...
def get_angle(p0: Point, p1: Point, p2: Point) -> float:
    ang = math.degrees(
        math.atan2(p1.y - p0.y, p1.x - p0.x) - math.atan2(p2.y - p0.y, p2.x - p0.x)
    )
        ...
        for pos_y in range(len(asteroids_map[pos_x])):
            ...
            if asteroids_map[pos_x][pos_y] == "#":
                ...
                asteroids_queue.append(
                    (
                        p,
                        get_distance(station, p),
                        get_angle(station, Point(0, station.y), p),
                    )
                )
