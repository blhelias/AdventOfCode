...
def test_diagnostic(
    ...
    while True:
        ...
        if (
            opcode_with_zeros[1] == "0"
            and opcode_with_zeros[-1] != "4"
            and opcode_with_zeros[-1] != "3"
        ):
            # Position mode
    ...
    for phases_settings in permutations(LOOP_MODE_PHASES):
        ...
        for phase in phases_settings:
            try:
                ...
                sequence, diagnostic, _ = test_diagnostic(
                    sequence, (str(phase), input_)
                )
    ...
    for phases_settings in permutations(LOOP_MODE_PHASES):
        ...
        while running:
            for amp_key in range(5):
                ...
                amp, diagnostic, status, pointer = test_diagnostic(
                    amplifiers_states[amp_key]["intcode"].split(","),
                    [str(input_)],
